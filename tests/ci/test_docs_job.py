...
class TestDocsJob:
    ...
    def test_docs_runs_mkdocs_build_strict(
        self, ci_workflow: dict[str, Any]
    ) -> None:
        docs = ci_workflow["jobs"]["docs"]
        run_steps = [s.get("run", "") for s in docs["steps"]]
        assert any(
            "uv run mkdocs build --strict" in step for step in run_steps
